...
module.exports.getOrdersForUserByStatus = async (status, user) => {
	return await Order.find({ status, user })
		.populate("user")
		.populate({
			path: "order_info",
			populate: {
...
module.exports.changeOrderStatus = async (_id, state) => {
	return await Order.updateOne({ _id }, { state: state }, { new: true });
};
