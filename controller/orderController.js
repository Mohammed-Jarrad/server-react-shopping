...
module.exports.getOrdersForUser = async (
  req = express.request,
  res = express.response
) => {
  try {
    const result = await orderService.getOrdersForUser(res.locals.userId);
    res.status(200).json({ orders: result });
  } catch (e) {
    const err = `faild to get Orders for user with id ${res.locals.userId}, err" ${e.message}`;
    res.status(400).json({ message: err });
  }
};
...
module.exports.deleteOrder = async (
  req = express.request,
  res = express.response
) => {
  try {
...
module.exports.updateOrder = async (
  ...
  try {
    const result = await orderService.updateById(req.params.id, req.body);
    res.status(200).json(result);
  } catch (e) {
